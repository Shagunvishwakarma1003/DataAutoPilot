...
def suggest_deep_learning(df: pd.DataFrame) -> dict:
    ...
    text_hint = any(
        k in colnames for k in ["text", "review", "comment", "message", "sentence"]
    )
    image_hint = any(
        k in colnames for k in ["image", "img", "filepath", "path", "file"]
    )
    ...
    if image_hint:
        return {
            "recommend": True,
            "type": "image",
            "reason": "Columns suggest image/file paths",
        }
    ...
    if text_hint:
        return {
            "recommend": True,
            "type": "text",
            "reason": "Columns suggest text data",
        }
    ...
    if big_data:
        return {
            "recommend": True,
            "type": "tabular_deep",
            "reason": "Very large dataset/features",
        }

    return {
        "recommend": False,
        "type": None,
        "reason": "Machine Learning is sufficient",
    }
