...
import pandas as pd
import argparse
from detector import detect_supervised_type,detect_unsupervised_type, suggest_deep_learning
from unsupervised import run_clustering, run_pca, generate_cluster_report, interpret_clusters, interpret_clusters_smart, auto_name_cluster, generate_business_recommendations
from association import run_association_rules
from supervised import train_regression,train_classification
from visualization import plot_k_silhouette, plot_pca_clusters, save_missing_values_plot, save_correlation_heatmap
from utils import validate_dataframe, validate_target, resolve_target_column, save_artifact, save_model, load_artifact, detect_target_column
...
def main():
    ...
    parser.add_argument('--target', required=False,
                        help= 'Target column (optional)')
    parser.add_argument('--use_deep', action='store_true',
                        help= 'Force deep learning mode (future)')
    parser.add_argument('--predict', type=str, default=None,
                        help='CSV path for prediction')
    parser.add_argument('--load-model', type=str, default=None,
                        help='Load saved .pkl model (used with --predict only)')
    parser.add_argument('--predict-on-train', action='store_true',
                        help='Predict on training dataset')
    parser.add_argument('--cv', type=int, default=0,
                        help='Number of CV folds (e.g., 5). 0 = no CV')
    ...
    if args.target:
        ...
        if args.load_model and args.predict:
            ...
            predict_to_csv(model, pred_df, target=None, save_path='output/predictions.csv')
    ...
    if args.target:
        ...
        if task == 'regression':
            ...
            if args. predict_on_train:
                predict_to_csv(best_model, df, target=args.target, save_path='output/predictions_train.csv')
                print('✅ Train predictions saved: output/predictions_train.csv')

            # Predict on new CSV (optional);
            if args.predict:
                new_df = pd.read_csv(args.predict)
                predict_to_csv(best_model, new_df, target=None, save_path='output/predictions_new.csv')
        ...
        if unsup_task == 'association_rules':
            ...
            # Save score summary
            pd.DataFrame(cl_out['scores']).to_csv('output/k_silhouette_scores.csv', index=False)

            # Save labels
            pd.DataFrame({'cluster_label':cl_out['labels']}).to_csv('output/clusters.csv', index=False)
            ...
            smart_lines = interpret_clusters_smart(df, report_df, save_path='output/cluster_interpretation.txt')
