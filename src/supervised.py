...
def train_regression(df: pd.DataFrame, target: str, random_state=42, cv=None):
    ...
    try:
        ...
        model_name = best_model.named_steps["model"]._class.name_

        tree_models = ["RandomForestRegressor", "XGBRegressor", "DecisionTreeRegressor"]

        if model_name in tree_models:

            shap_out = explain_with_shap(
                best_model, x_small, out_dir="output/shap", max_display=10
            )
            ...
            print("SHAP skipped (model not supported)")
...
def train_classification(df: pd.DataFrame, target: str, random_state=42, cv=None):
    ...
    )  # Convert to numeric codes for classification

    use_cw, min_ratio = should_use_class_weight(y)

    if use_cw:
        print(
            f"⚠️ Imbalance detected (minority={min_ratio:.2f}) → enabling class_weight='balanced'"
        )
    else:
    ...
    models = [
        (
            "LogisticRegression",
            LogisticRegression(
                max_iter=2000, class_weight="balanced" if use_cw else None
            ),
        ),
        (
        ...
        (
            ...
            DecisionTreeClassifier(
                random_state=random_state, class_weight="balanced" if use_cw else None
            ),
        ),
        ("SVM", SVC(probability=True, class_weight="balanced" if use_cw else None)),
    ...
    top2 = (
        ...
        else [results_sorted[0]["model"], results_sorted[1]["model"]]
    )
    ...
    best_after_tune_cv = get_metric(
        results_sorted[0], "cv_f1", "cv_f1_score", default=None
    )
    ...
    for nm in top2:
        try:
            ...
            current_best_test_f1 = float(
                get_metric({"x": None}, "x", default=-1)
            )  # dummy
            if best_after_tune is best_model:
                # set baseline from best_model on test
                preds_base = best_model.predict(x_test)
                base_f1 = float(
                    f1_score(y_test, preds_base, average=avg_type, zero_division=0)
                )
                current_best_test_f1 = base_f1
            else:
                current_best_test_f1 = max(
                    [t.get("test_f1", -1) for t in tune_logs if t.get("tuned")],
                    default=-1,
                )
