...
def generate_cluster_report(df, labels, save_path="output/cluster_report.csv"):
    ...
    size_df = df_Copy["cluster"].value_counts().sort_index().reset_index()
    ...
    mean_df = df_Copy.groupby("cluster")[numerical_cols].mean().reset_index()
...
def interpret_clusters_smart(
    df, report_df, top_n=3, save_path="output/cluster_interpretation.txt"
):
    """
    ...
    for _, row in report_df.iterrows():
        ...
        numeric_in_report = [
            ...
            if c in overall.index and c not in ["cluster", "count", "percentage"]
        ]
        devs = []
        ...
        if insights:
            ...
            lines.append(
                "  Insights: No strong deviations found (clusters are similar)."
            )
...
def generate_business_recommendations(
    cluster_names, report_df, save_path="output/business_recommendation.txt"
):
    """
    ...
    for _, row in report_df.iterrows():
        ...
        if charges is not None and charges >= 20000:
            lines.append(
                "- Recommendation: Offer premium plans, upsell high coverage polices."
            )
            lines.append(
                "- Action: Focus on retention + personalized suport (high value customer)."
            )

        if bmi is not None and bmi >= 30:
            lines.append(
                "- Recommendation: Add wellness programs+ preventive health offers."
            )
            lines.append("- Action: Risk reduction through fitness/health incentives.")

        if age is not None and age >= 45:
            lines.append(
                "- Recommendation: Senior-friendly policy bundles + long-term coverage options."
            )
            lines.append(
                "- Action: Target with long-term plans, claim support benefits."
            )

        # fallback
        if charges is None and bmi is None and age is None:
            lines.append(
                "- Recommendation: No strong signals; test marketing offers based on other features."
            )
